...
function changeCustomQtyOnClick(Event) {
    var that;
    if (this.classList.contains('bootstrap-touchspin-up')) {
        var currentInput = this.closest('.price-review .input-group').querySelector('.custom_quantity');
    ...
    newQuantity = parseInt($(that).val());
    var randomId = 0;
    var idCombination = 0;
    // in calse dropdown + show image are enabled
    if ($(that).parent().parent().find('.dd-selected-value').length > 0)
    {
        randomId = $(that).parent().parent().find('.randomid-group').data('randomid');
        var idGroup = $(that).parent().parent().find('.randomid-group').data('idgroup');
        idCombination = $('#combination_' + idGroup).find('.dd-selected-value').val();
    } else {
        randomId = typeof $(that).parents('tr').find(':checked').data('randomid') !== 'undefined' ? $(this).parents('tr').find(':checked').data('randomid') : $(this).parents('tr').find(PriceTable.instance._selectors.accessoryItem).data('randomid');
        idCombination = typeof $(that).parents('tr').find(':checked').data('id-product-attribute') !== 'undefined' ? $(this).parents('tr').find(':checked').data('id-product-attribute') : $(this).parents('tr').find(PriceTable.instance._selectors.accessoryItem).data('id-product-attribute');
    }
...
function changeCustomQty(Event) {
...
var PriceTable = function (options)
{
    ...
    this.onLoad = function ()
    {
        ...
        if ($('#product' + ' ' + this._selectors.groupAccessories + ' ' + this._selectors.accessoriesCustomQuantity).length > 0)
        {
            ...
            /*****************************************************************************************************/
            $(this._selectors.accessoriesCustomQuantity).parent().on('mouseenter', function () {
                // Store the current value on focus and on change
                previousAccessoryQuantity = $(this).find('.custom_quantity').val();
            });
            $(this._selectors.accessoriesCustomQuantity).on('change', changeCustomQty);
            $(this._selectors.accessoriesCustomQuantity)
                .closest('.price-review .input-group').find('button.btn-touchspin')
                .on('click', changeCustomQtyOnClick);
        }
    };

				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						...
						{if $smarty.foreach.categoryTreeBranch.first}  <ul class="nav nav-tabs">{/if} 
							<li class="nav-item">
								<a href="#content-tab-{$child.elementId}"  {if isset($child.tabtitle)}title=""{/if} data-toggle="tab">{if isset($child.tabtitle)}{$child.tabtitle nofilter}{else}{l s='Set tab title'}{/if}</a>
							</li>
					...
					{foreach from=$node.children item=child name=categoryTreeBranch}
						{if $child.type==3}
						{if $smarty.foreach.categoryTreeBranch.first}  <div class="tab-content"> <div id="tabs">{/if} 
							{include file="./front_content.tpl" node=$child }
						{if $smarty.foreach.categoryTreeBranch.last}  </div> </div> {/if} 
						{else}
							{include file="./front_content.tpl" node=$child }
						{/if}
					{/foreach}

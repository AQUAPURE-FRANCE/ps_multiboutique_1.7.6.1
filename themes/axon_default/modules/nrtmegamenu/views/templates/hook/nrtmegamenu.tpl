                ...
                <li class="root root-{$root->id_nrtmegamenu} {$root->menu_class}">
                    <div class="root-item{if $root->description == ''} no-description{/if}">
                        {if $root->link != ''}<a href="{$root->link}" {if $root->open_in_new}target="_blank"{/if}>{/if}
                            <div class="title title_font">{if $root->icon_class != ''}<span class="{$root->icon_class}"></span>{/if}<div class="title-menu">{$root->title nofilter}</div>{if isset($root->items)}<span class="icon-has-sub fa fa-angle-down"></span>{/if}</div>

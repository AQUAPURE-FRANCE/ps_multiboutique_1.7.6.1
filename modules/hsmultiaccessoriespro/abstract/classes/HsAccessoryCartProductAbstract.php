...
class HsAccessoryCartProductAbstract extends ObjectModel
{
     ...
     */
    public function __construct($id_cart, $id_product, $id_main_product_attribute, $id_accessory, $id_accessory_attribute, $id_customization = 0)
    {
        $id_accessory_cart_product = self::getId($id_cart, $id_product, $id_main_product_attribute, $id_accessory, $id_accessory_attribute, $id_customization);
     ...
     * @return int
     */
    protected static function getId($id_cart, $id_product, $id_main_product_attribute, $id_accessory, $id_accessory_attribute, $id_customization)
    {
        $cache_key = implode('_', array($id_cart, $id_product, $id_main_product_attribute, $id_accessory, $id_accessory_attribute, $id_customization));
    ...
    {
        $query = new DbQuery();
        $query->select('`id_accessory_cart_product`, `id_product`, `id_cart`, `id_product_attribute`, `id_accessory`, `id_accessory_attribute`, `id_customization`, `quantity`, `prev_quantity`');
        $query->from('accessory_cart_product');
        $query->where('`id_cart` = ' . (int) $id_cart);
        $query->where('`id_product` = ' . (int) $id_product);
     ...
     * @return array
     */
    public static function getAccessoriesInCart($id_cart, $id_product, $id_product_attribute)
    {
        $cache_id = __CLASS__ . '::' . __FUNCTION__ . '_' . $id_cart . '_' . $id_product . '_' . $id_product_attribute;
        if (!Cache::isStored($cache_id)) {
            $query = new DbQuery();
            $query->select('DISTINCT acp.`id_accessory_cart_product`, acp.`id_product`, acp.`id_cart`, acp.`id_product_attribute`, acp.`id_accessory`, acp.`id_accessory_attribute`, acp.`id_customization`, acp.`quantity`, acp.`prev_quantity`, agp.`default_quantity`');

...
{if isset($breadcrumb)}
 <nav data-depth="{$breadcrumb.count}" {if $NRT_IMG_BREADCRUMB}style="background:url({$NRT_IMG_BREADCRUMB}) center center"{/if} class="breadcrumb hidden-sm-down {if $NRT_IMG_BREADCRUMB}bg_breadcrumb{/if}">
  <ol itemscope itemtype="http://schema.org/BreadcrumbList">
    {if $title_category != ''}
    {assign var="link_category" value=null}
    {$link_category.id_category = $id_category}
    {$link_category.slug = $cat_link_rewrite}
     <li  class="title_large">{$title_category}</li>
    {else}
     <li  class="title_large">{l s='All Blog News' mod='smartblog'}</li>
    {/if}

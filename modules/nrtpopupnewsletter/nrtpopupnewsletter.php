...
class NrtPopupNewsletter extends Module
{
	private $_html = '';
    ...
    function __construct()
    {
		...
		$this->displayName = $this->l('Popup Newsletter');
	...
	{		
		...
		$this->context->controller->getLanguages();
                ...
                foreach ($this->context->controller->_languages as $language){
                    $title[(int)$language['id_lang']] = '<p>Join Us Now!</p>';
                    $content[(int)$language['id_lang']] = '<p>And be the first to get the scoop on sales exclusive offers <br />and new arrivals.</p>';
                }
                
                $this->_createTab();
		if (
			parent::install() == false
			|| $this->registerHook('popupnewsletter') == false
			|| $this->registerHook('header') == false
			|| Configuration::updateValue('NRT_TITLE', $title,true) == false
			|| Configuration::updateValue('NRT_TEXT', $content,true) == false
			|| Configuration::updateValue('NRT_WIDTH', 770) == false
			|| Configuration::updateValue('NRT_HEIGHT', 450) == false
			|| Configuration::updateValue('NRT_NEWSLETTER', true) == false
			|| Configuration::updateValue('NRT_BG', true) == false
			|| Configuration::updateValue('NRT_BG_IMAGE', _MODULE_DIR_.$this->name.'/img/background_image1.jpg') == false
			|| Configuration::updateValue('NRT_COUNTDOWN_POPUP', '2016-08-10 03:38:00') == false
			|| Configuration::updateValue('NRT_COUNTDOWN_POPUP_START', '0000-00-00 00:00:00') == false
                    )
			return false;
		return true;
	}
	...
	public function uninstall()
	{
		$this->context->controller->getLanguages();
		foreach ($this->context->controller->_languages as $language){
			Configuration::deleteByName('NRT_TEXT_'.(int)$language['id_lang']);
                        Configuration::deleteByName('NRT_TITLE_'.(int)$language['id_lang']);
                }
                $this->_deleteTab();
		return 
			Configuration::deleteByName('NRT_WIDTH') &&
			Configuration::deleteByName('NRT_HEIGHT') &&
        ...
        private function _createTab()
        {
            ...
            $tab->name = array();
            foreach (Language::getLanguages() as $lang) {
                $tab->name[$lang['id_lang']] = "Manage Popup newsletter";
            }
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('nrt_submit')) {
			Configuration::updateValue('NRT_WIDTH', (int)Tools::getValue('NRT_WIDTH'));
			Configuration::updateValue('NRT_HEIGHT', (int)Tools::getValue('NRT_HEIGHT'));
	...
	public function hookpopupnewsletter($params)
	{
	...
	public function hookHeader($params)
	{
		if (isset($this->context->controller->php_self) && $this->context->controller->php_self == 'index') {
			...
			Media::addJsDef(array('nrt_path' => Tools::getShopProtocol().Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri.'modules/nrtpopupnewsletter/ajax.php' ));
		}
	}
	...
	{
		$values = array(
			'NRT_WIDTH' => Tools::getValue('NRT_WIDTH', Configuration::get('NRT_WIDTH')),
			'NRT_HEIGHT' => Tools::getValue('NRT_HEIGHT', Configuration::get('NRT_HEIGHT')),
			...
			'NRT_COUNTDOWN_POPUP_START' => Tools::getValue('NRT_COUNTDOWN_POPUP_START', Configuration::get('NRT_COUNTDOWN_POPUP_START')),
                ...
                foreach (Language::getLanguages(false) as $lang){
                            $values['NRT_TITLE'][(int)$lang['id_lang']] = Configuration::get('NRT_TITLE', (int)$lang['id_lang']);
                            $values['NRT_TEXT'][(int)$lang['id_lang']] = Configuration::get('NRT_TEXT', (int)$lang['id_lang']);
                }
		return $values;
	}
	...
	public function getConfigFromDB()
	{
		...
		return array(
			'NRT_WIDTH' => (Configuration::get('NRT_WIDTH') ? Configuration::get('NRT_WIDTH'): "400"),
			'NRT_HEIGHT' => (Configuration::get('NRT_HEIGHT') ? Configuration::get('NRT_HEIGHT'): "400"),
			'NRT_NEWSLETTER' => (Configuration::get('NRT_NEWSLETTER') ? Configuration::get('NRT_NEWSLETTER'): false),
			'NRT_TEXT' => (Configuration::get('NRT_TEXT', $this->context->language->id) ? Configuration::get('NRT_TEXT', $this->context->language->id): false),
                        'NRT_TITLE' => (Configuration::get('NRT_TITLE', $this->context->language->id) ? Configuration::get('NRT_TITLE', $this->context->language->id): false),
			...
			'NRT_PATH' => Tools::getShopProtocol().Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri.'modules/nrtpopupnewsletter/ajax.php',
	...
	{
		return Mail::Send($this->context->language->id, 'nrtnewsletter_voucher', Mail::l('Newsletter voucher', $this->context->language->id), array('{discount}' => $code), $email, null, null, null, null, null, dirname(__FILE__).'/mails/', false, $this->context->shop->id);
	}

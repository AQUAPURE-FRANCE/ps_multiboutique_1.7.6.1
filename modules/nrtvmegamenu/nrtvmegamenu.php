...
require (dirname(__FILE__).'/classes/nrtvmegamenu.class.php');

class nrtvmegamenu extends Module {
	...
	private function uninstallDb() {
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'nrt_vmegamenu_menus`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'nrt_vmegamenu_submenus`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'nrt_vmegamenu_menus_lang`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'nrt_vmegamenu_links`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'nrt_vmegamenu_links_lang`');
		return true;
	}
...
// Check for parent tab2
            ...
            $tab = new Tab();
            $tab->active = 1;
            $tab->class_name = "AdminNrtVMegaMenu";
            $tab->name = array();
            foreach (Language::getLanguages() as $lang) {
        ...
        {
            $id_tab = Tab::getIdFromClassName('AdminNrtVMegaMenu');
            $parentTabID = Tab::getIdFromClassName('AdminNrtMenu');
...
// Get the number of tabs inside our parent tab
	...
	public function getContent() {
		...
		if (Tools::isSubmit('submitMenuTabRemove')) {
			$id_menu = Tools::getValue('id_nrt_vmegamenu_menus', 0);
			...
			$id_nrt_vmegamenu_links = Tools::getValue('id_nrt_vmegamenu_links', 0);
			VMegaMenuItem::removeLink($id_nrt_vmegamenu_links, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKTOPMENU_ITEMS', str_replace(array('LNK' . $id_nrt_vmegamenu_links . ',', 'LNK' . $id_nrt_vmegamenu_links), '', Configuration::get('MOD_BLOCKTOPMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		} else if (Tools::isSubmit('editLink')) {
			$id_nrt_vmegamenu_links = (int)Tools::getValue('id_nrt_vmegamenu_links', 0);
			$id_shop = (int)Shop::getContextShopID();

			if (!Tools::isSubmit('link')) {				
				$link_params = VMegaMenuItem::getCustomLinkLang($id_nrt_vmegamenu_links, $id_shop);
			} else {
				VMegaMenuItem::updateLink(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_nrt_vmegamenu_links, (int)$id_nrt_vmegamenu_links);
			...
			foreach ($links as $link) {
				...
				$this->_html .= '
					<div class="list_menuitem">
						'.(isset($link['active']) && ($link['active'] == 1) ? '<img src="'.$this->_path.'img/act.png" />' : ' <img src="'.$this->_path.'img/pas.png" width="auto" height="auto" />').'<span>'.Tools::safeOutput($itemData['name']).'</span>
							<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
								<input type="hidden" name="id_nrt_vmegamenu_menus" id="id_menu" value="'.(int)$link['id_nrt_vmegamenu_menus'].'" />
		...
		<fieldset class="menuItem" data-shop-id="'.(int)Shop::getContextShopID().'">
			...
			if (count($links)) {
				...
				<div class="list_menuitem customlinks">
					...
					<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
						<input type="hidden" name="id_nrt_vmegamenu_links" value="'.(int)$link['id_nrt_vmegamenu_links'].'" />
			...
			$this->_html .= '<label style="clear: both;">'.$this->l('New Window').'</label>
				...
				<div class="margin-form">';
					if (Tools::isSubmit('id_nrt_vmegamenu_links'))
						$this->_html .= '<input type="hidden" name="id_nrt_vmegamenu_links" value="'.(int)Tools::getValue('id_nrt_vmegamenu_links').'" />';
	...
	public function displayAddTabForm($id_menu = NULL) {
		...
		$this->_html .= '<fieldset class="menuItem" data-shop-id="'.(int)Shop::getContextShopID().'">';
			...
			if (isset($id_menu)) {
				$this->_html .= '<input type="hidden" name="id_nrt_vmegamenu_menus" id="id_menu" value="'.$id_menu.'" />';
			} else {
				$this->_html .= '<input type="hidden" name="id_nrt_vmegamenu_menus" id="id_menu" value="NONE" />';
			}
			...
			<div class="margin-form">
				<select id="topLevelLinks" data-lang="'.$id_lang.'" style="height:30px">';
					...
					foreach ($links as $link) {
						if ($link['label'] == '') {
							$link = VMegaMenuItem::getLink($link['id_nrt_vmegamenu_links'], $default_language, (int)Shop::getContextShopID());
							$this->_html .= '<option '.(isset($link_params['label']) ? ($link_params['label'] == 'LNK'.(int)$link[0]['id_nrt_vmegamenu_links'] ? 'selected' : '') : '').' value="LNK'.(int)$link[0]['id_nrt_vmegamenu_links'].'">'.$spacer.$link[0]['label'].'</option>';
						} else
							$this->_html .= '<option '.(isset($link_params['label']) ? ($link_params['label'] == 'LNK'.(int)$link['id_nrt_vmegamenu_links'] ? 'selected' : '') : '').' value="LNK'.(int)$link['id_nrt_vmegamenu_links'].'">'.$spacer.$link['label'].'</option>';
					}
					$this->_html .= '</optgroup>';
					...
					$.ajaxSetup({
				        type: "POST",
				        url: "'.Tools::getShopProtocol().Context::getContext()->shop->domain.Context::getContext()->shop->physical_uri.'modules/nrtvmegamenu/ajax/queries.php",
			...
			<div class="after-label-add-content'.(isset($link_params['label']) ? '' : ' hide').'">
				...
				<div class="margin-form activeLabel">
					<div id="main_image_upload" data-type-content="image">
						...
						<img id="main_image_prev" src="'.(isset($link_params["main_image"]) && $link_params["main_image"] != "" ? $this->_path.'uploads/'.$link_params["main_image"] : $this->_path.'img/demo.jpg').'" alt="" class="mainimagepreview previewImage" width="auto" height="auto" />						
						...
						<div class="cleft_panel cpanel'.(isset($section_settings["state_left"]) && ($section_settings["state_left"] == 1) ? ' active' : '').'" data-section-name="left">
							...
							<div id="left-panel-container" class="panel-container">
								...
								<div id="left_image_upload" class="margin_field '.(isset($section_settings["left"]) && ($section_settings["left"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									...
									<img id="left_image_prev" src="'.(isset($section_settings['left_image']) && $section_settings['left_image'] != "" ? $this->_path.'uploads/'.$section_settings['left_image'] : $this->_path.'img/demo.jpg').'" alt="" class="leftimagepreview previewImage" width="auto" height="auto" />
									...
									if(isset($section_settings['left_products'])){
										...
										foreach ($lfprdns as $lfprdn) {
											...
											if (Validate::isLoadedObject($lproduct)) {
												...
												$this->_html .= '<li data-value="PRD'.$lid.'"><img src="'.$this->getProductCover($lid).'" alt="" width="auto" height="auto" />'.$nm.'<br/><span class="removeThis"></span></li>'.PHP_EOL;
											}
											}
										}		
									$this->_html .= '
								...
								<div id="mainlinks_links" class="margin_field '.(isset($section_settings["main"]) && ($section_settings["main"] == "LINKS") ? '' : 'hide').'" data-type-content="main-links">
									...
									<div class="shop_source">										
										...
										foreach ($links as $link) {
											if ($link['label'] == '') {
												$link = VMegaMenuItem::getLink($link['id_nrt_vmegamenu_links'], $default_language, (int)Shop::getContextShopID());
												$this->_html .= '<option value="LNK'.(int)$link[0]['id_nrt_vmegamenu_links'].'">'.$spacer.$link[0]['label'].'</option>';
											} else
												$this->_html .= '<option value="LNK'.(int)$link['id_nrt_vmegamenu_links'].'">'.$spacer.$link['label'].'</option>';
										}
										$this->_html .= '</optgroup>';
										$this->_html .= '</select>
									</div>
										...
										if(isset($section_settings['main_products'])){
											...
											foreach ($main_vals as $val) {
												...
												if (Validate::isLoadedObject($mproduct)) {
													...
													$this->_html .= '<li data-value="PRD'.$mid.'"><img src="'.$this->getProductCover($mid).'" alt="" width="auto" height="auto" />'.$nm.'<br/><span class="removeThis"></span></li>'.PHP_EOL;
												}
												}
											}		
										$this->_html .= '
							...
							<div id="right-panel-container" class="panel-container">
								...
								<div id="right_image_upload" class="margin_field '.(isset($section_settings["right"]) && ($section_settings["right"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									<form method="post" enctype="multipart/form-data"  action="'.$this->_path.'ajax/upload.php">  
						       		...
						       		<strong>'.$this->l('Recommended image width is 180px').'</strong>	
									<img id="right_image_prev" src="'.(isset($section_settings["right_image"]) && $section_settings["right_image"] != "" ? $this->_path.'uploads/'.$section_settings["right_image"] : $this->_path.'img/demo.jpg').'" alt="" class="rightimagepreview previewImage" width="auto" height="auto" />
									...
									if(isset($section_settings["right_products"])) {
										...
										foreach ($right_vals as $val) {
											...
											if (Validate::isLoadedObject($product)) {
												...
												$this->_html .= '<li data-value="PRD'.$id.'"><img src="'.$this->getProductCover($id).'" alt="" width="auto" height="auto" />'.$nm.'<br/><span class="removeThis"></span></li>'.PHP_EOL;
											}
											}
										}		
									$this->_html .= '
						...
						<div class="cbottom_panel cpanel'.(isset($section_settings["state_bottom"]) && ($section_settings["state_bottom"] == 1) ? ' active' : '').'" data-section-name="bottom">
							...
							<div id="bottom-panel-container" class="panel-container">
								...
								<div id="bottom_image_upload" class="margin_field '.(isset($section_settings["bottom"]) && ($section_settings["bottom"] == "IMAGE") ? '' : 'hide').'" data-type-content="image">
									...
									<img id="bottom_image_prev" src="'.(isset($section_settings["bottom_image"]) && $section_settings["bottom_image"] != "" ? $this->_path.'uploads/'.$section_settings["bottom_image"] : $this->_path.'img/demo.jpg').'" alt="" class="bottomimagepreview previewImage" width="auto" height="auto" />
				...
				case 'LNK' :
					...
					if (count($link)) {
						if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
							...
							$link = VMegaMenuItem::getCustomLink($link[0]['id_nrt_vmegamenu_links'], (int)$default_language, (int)Shop::getContextShopID());							
						}
						$this->_html .= '<li data-value="LNK' . $link[0]['id_nrt_vmegamenu_links'] . '">' . $link[0]['label'] . '<span class="removeThis"></span></li>';
					}
					break;
				case 'SHOP' :
			...
			switch ($type)
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = VMegaMenuItem::getCustomLink($link[0]['id_nrt_vmegamenu_links'], $default_language, (int)Shop::getContextShopID());
						}
	...
	private function getProductImage($id)
	{
		...
		if (!is_null($product->id))
		$this->_menu .= '<li><div class="li-indent"><a class="product-image-link" href="'.htmlentities($product->getLink()).'"><img src="'.$this->context->link->getImageLink($product->link_rewrite, (int)$id.'-'.$cover['id_image'], 'home_'.$this->imageSuffix).'" alt="'.$nm.'" width="auto" height="auto" /></a>';
	...
	{	
		...
		$isr++;
			...
			$this->_menu .= '<li class="v-megamenuitem id_menu'.$link['id_nrt_vmegamenu_menus'].(($submenu) ? ' hasChildren' : '').(($link['narrow'] == 1)  ? ' narrow' : '').(($this->curPath() == $itemData['link']) ? ' current-item' : '').(($isr>$sl_ct && $kt>$sl_ct ) ? ' more_here" style="display:none;"' : '"').' data-menuid="'.$link['id_nrt_vmegamenu_menus'].'"><a class="menu-title" href="'.$itemData['link'].'" '.(((!empty($itemData['new_window'])) && ($itemData['new_window'] == 1)) ? ' target="_blank"' : '' ).'>'.(($link['main_icon'] != "")  ? '<i class="'.$link['main_icon'].'"></i>': '').'<span class="item-icon">'.$itemData['name'].'</span></a>'.(($submenu) ? '<a href="javascript:void(0)" class="opener"></a><div class="navbar-toggler collapse-icons hidden-lg-up" data-toggle="collapse" data-target="#vmegamenu_'.$link['id_nrt_vmegamenu_menus'].'"><i class="fa fa-plus add"></i><i class="fa fa-minus remove"></i></div>' : '').'';				
			if ($submenu) {
			...
			$this->_menu .='<div id="vmegamenu_'.$link['id_nrt_vmegamenu_menus'].'" class="collapse submenu dd-section'.$column.' fmsid'.$link['id_nrt_vmegamenu_menus'].' clearfix" '.(($link['main_image'] != "")  ? ' style="background-image:url('.$this->context->link->getMediaLink(_MODULE_DIR_.'nrtvmegamenu/uploads/'.$link['main_image']).')"' : '').'><div class="sections-contaier">';
				...
				if ($link['state_right'] == 1) {
					...
					switch ($link['right']) {				
						...
						case 'IMAGE':
							...
							$this->_menu .='<a href="'.$link['right_image_link'].'" class="imagelink"><img src="'.$this->context->link->getMediaLink(_MODULE_DIR_.'nrtvmegamenu/uploads/'.$link['right_image']).'" class="rightimage menu-image" alt="" width="auto" height="auto" /></a>';
							break;
						case 'VID':
				...
				if ($link['state_bottom'] == 1) {
					...
					switch ($link['bottom']) {
						...
						if (isset($link['bottom_image']) && !empty($link['bottom_image']))
							$this->_menu .='<a href="'.$link['bottom_image_link'].'" class="imagelink"><img src="'.$this->context->link->getMediaLink(_MODULE_DIR_.'nrtvmegamenu/uploads/'.$link['bottom_image']).'" class="bottomimage menu-image" alt="" width="auto" height="auto" /></a>';
						break;
						case 'LINKS':
							...
							if ($link['bottom_carousel'] == 1)
								$this->_menu .= '<script>$(".id_menu'.$link['id_nrt_vmegamenu_menus'].' .v-bottom-section-links").flexisel({pref: "vm-man",visibleItems: 5,animationSpeed: 500,autoPlay: true,autoPlaySpeed: 3900,pauseOnHover: true,enableResponsiveBreakpoints: true,clone : true, responsiveBreakpoints: {portrait: { changePoint:400,visibleItems: 1}, landscape: { changePoint:768,visibleItems: 2},tablet: { changePoint:991,visibleItems: 3},tablet_land: { changePoint:1199,visibleItems: 5}}});</script>';
						break;
						case 'VID':
			...
			$this->_menu .= '</li>'.PHP_EOL;
			if($isr==$kt && $isr > $sl_ct){ $this->_menu .="<li  class='more-vmegamenu'><span><i class='fa fa-plus'></i>" .$this->l('More Categories'). "</span></li>";  }				
		}	
	}
	...
	public function hookHeader($params) {
            ...
            $this->context->controller->addJS($this->_path.'js/vmegamenu.js');
			Media::addJsDef(array('CloseVmenu' => $this->l('Close Menu')));
			Media::addJsDef(array('MoreVmenu' => $this->l('More Categories')));
	}
    ...
    public function hookVmegamenu($params) {
		...
		return $this->display(__FILE__, 'nrtvmegamenu.tpl');
	}
	
	function hookRightColumn($params)
	{
		return $this->hookVmegamenu($params);
	}

...
class BlockWishList extends Module
{
	...
	public function install($delete_params = true)
	{
		...
		if (!parent::install() ||
			!$this->registerHook('linkwishlist') ||
	...
	{
		if (($delete_params && !$this->deleteTables()) || !parent::uninstall() || !$this->_deleteTab())
			return false;
...
public function addTab()
	{
        ...
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminNrtWishtList";
        $tab->name = array();
        foreach (Language::getLanguages() as $lang) {
            $tab->name[$lang['id_lang']] = "Manage Block Wishlist";
        }
    ...
    {
        $id_tab = Tab::getIdFromClassName('AdminNrtWishtList');
		$parentTabID = Tab::getIdFromClassName('AdminNrtMenu');
        $tab = new Tab($id_tab);
        $tab->delete();
	...
	public function hookLinkwishlist($params)
	{
		return ($this->display(__FILE__, 'linkwishlist.tpl'));
	}
	...
	public function hookHeader($params)
	{
		...
		if ($this->context->customer->isLogged())
		{
			...
			Media::addJsDef(array('isLogged' => true));
			Media::addJsDef(array('isLoggedWishlist' => true));	
			Media::addJsDef(array('wishlistProductsIds' => ($id_wishlist == false ? false : WishList::getProductByIdCustomer($id_wishlist,$this->context->customer->id, $this->context->language->id, null, true))));	
			
		}
		else
		{
			Media::addJsDef(array('isLogged' => false));
			Media::addJsDef(array('isLoggedWishlist' => false));	
			Media::addJsDef(array('wishlistProductsIds' => false));	
			$this->smarty->assign(array('wishlist_products' => false, 'wishlists' => false));
			
		}
			Media::addJsDef(array('view_wishlist' => $this->l('Go to Wishlist')));
			Media::addJsDef(array('loggin_required' => $this->l('You must be logged in to manage your wishlist.')));
			Media::addJsDef(array('added_to_wishlist' => $this->l('The product was successfully added to your wishlist.')));
			Media::addJsDef(array('mywishlist_url' =>  $this->context->link->getModuleLink('blockwishlist', 'mywishlist')));

...
class NrtBlockSearch extends Module
{
	private $_prefix;
    private $_fields_form = array();
	public function __construct()
	{
		$this->name = 'nrtblocksearch';
		$this->tab = 'search_filter';
		$this->version = "2.0";
		$this->author = 'AxonVIP';
		$this->need_instance = 0;
		$this->_prefix = 'nrtsearch';
		$this->bootstrap = true;
		parent::__construct();
	...
	public function install()
	{
		$this->checkOwnerHooks();
                ...
                foreach ($this->context->controller->_languages as $language){
                    $tags[(int)$language['id_lang']] = '';
                }
		if (!parent::install() || !$this->_createTab() || !$this->registerHook('top') || !$this->registerHook('header') || !$this->registerHook('displayMobileTopSiteMap') || !Configuration::updateValue('NRTSEARCH_SHOW_CAT', 1) || !Configuration::updateValue('NRTSEARCH_SHOW_TAGS',0) || !Configuration::updateValue('NRTSEARCH_TAGS', $tags))
			return false;
		return true;
	}
        ...
        public function uninstall() {
            ...
            if (!parent::uninstall()
                || !$this->_deleteTab()
                || !Configuration::deleteByName('NRTSEARCH_SHOW_CAT')
                || !Configuration::deleteByName('NRTSEARCH_SHOW_TAGS')
                || !Configuration::deleteByName('NRTSEARCH_TAGS')
                )
                return false;
            return true;
        }
...
// Check for parent tab2
        ...
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminNrtBlockSearch";
        $tab->name = array();
        foreach (Language::getLanguages() as $lang) {
            $tab->name[$lang['id_lang']] = "Manage BlockSearch";
        }
    ...
    {
        $id_tab = Tab::getIdFromClassName('AdminNrtBlockSearch');
        $parentTabID = Tab::getIdFromClassName('AdminNrtMenu');
...
// Get the number of tabs inside our parent tab
    ...
    public function getContent() {
    	...
    	if ( Tools::isSubmit('submitNrtBlockSearch') ) {

            $this->makeFormConfig();
            $this->batchUpdateConfigs();
            $this->_clearCache('nrtblocksearch-top.tpl');
    ...
    public function renderForm() {
        ...
        $helper->submit_action = 'submitNrtBlockSearch';
	...
	{
		$key = $this->getCacheId('nrtblocksearch-top'.((!isset($params['hook_mobile']) || !$params['hook_mobile']) ? '' : '-hook_mobile'));
		if (Tools::getValue('search_query') || !$this->isCached('nrtblocksearch-top.tpl', $key))
		{
			$this->calculHookCommon($params);
			$this->smarty->assign(array(
				'nrtblocksearch_type' => 'top',
				'search_query' => (string)Tools::getValue('search_query'),
                                'NRTSEARCH_SHOW_CAT' => Configuration::get('NRTSEARCH_SHOW_CAT')
				)
			);
		}
		Media::addJsDef(array('nrtblocksearch_type' => 'top'));
		return $this->display(__FILE__, 'nrtblocksearch-top.tpl', Tools::getValue('search_query') ? null : $key);
	}
	...
	{
		if (file_exists(_PS_THEME_DIR_.'modules/nrtblocksearch/nrtblocksearch-instantsearch.tpl'))
			$this->smarty->assign('instance_tpl_path', _PS_THEME_DIR_.'modules/nrtblocksearch/nrtblocksearch-instantsearch.tpl');
		else
			$this->smarty->assign('instance_tpl_path', _PS_MODULE_DIR_.'nrtblocksearch/nrtblocksearch-instantsearch.tpl');
		$tags = '';
                $arrtags = array();
		...
		$this->smarty->assign(array(
            ...
            'form_link' => $this->context->link->getModuleLink('nrtblocksearch', 'search'),
	...
	{
        if(!Configuration::get('NRTSEARCH_SHOW_CAT'))
        	return '';
        ...
        $this->smarty->assign('nrtblocksearchCategTree', $blockCategTree);
        ...
        if (file_exists(_PS_THEME_DIR_.'modules/nrtblocksearch/categories.tpl'))
            $this->smarty->assign('nrtbranche_tpl_path', _PS_THEME_DIR_.'modules/nrtblocksearch/category-tree-branch.tpl');
        else
            $this->smarty->assign('nrtbranche_tpl_path', _PS_MODULE_DIR_.'nrtblocksearch/category-tree-branch.tpl');

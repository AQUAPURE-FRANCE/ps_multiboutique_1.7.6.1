{if $nrtmmMenuItems.items}

    {$depth = 1}
    <ol class="sortable">

        {foreach $nrtmmMenuItems.items as $menuItem}
            ...
            {if ($menuItem@index != $nrtmmMenuItems.count - 1) && ($nrtmmMenuItems.items[($menuItem@index + 1)]->depth <= $menuItem->depth)}
                </li>
            {/if}

            {$depth = $menuItem->depth}

            {if ($menuItem@index == $nrtmmMenuItems.count - 1)}

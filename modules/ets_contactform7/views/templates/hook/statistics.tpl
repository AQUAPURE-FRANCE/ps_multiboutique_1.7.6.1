...
<script type="text/javascript">
    var text_add_to_black_list ='{l s='Add IP address to backlist successful' js='1' mod='ets_contactform7' }';
                        ...
                        <form id="ctf_admin_filter_chart" class="defaultForm form-horizontal" action="{$action|escape:'quotes'}" enctype="multipart/form-data" method="POST">
                            <div class="ctf_admin_filter_chart_settings">
                                    ...
                                    <div class="ctf_admin_filter_button">
                                        <button name="submitFilterChart" class="btn btn-default" type="submit">{l s='Filter' mod='ets_contactform7'}</button>
                                        {if $show_reset}
                                            <a href="{$action|escape:'quotes'}" class="btn btn-default">{l s='Reset' mod='ets_contactform7'}</a>
                                        {/if}
                                    </div>
                            </div>
                        </form>
                ...
                <div class="ctf_admin_log form-group form_group_contact view-log">
                    {if $logs}
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {if $log.enable_form_page}
                                                <a  href="{Ets_contactform7::getLinkContactForm($log.id_contact|intval)|escape:'html':'UTF-8'}" class="dropdown-item product-edit" target="_blank" title="{$log.title|escape:'html':'UTF-8'}">
                                            {/if}
                                        ...
                                        <td class="statitics_form_action">
                                            <a class="btn btn-default view_location" title="{l s='View location' mod='ets_contactform7'}" href="https://www.infobyip.com/ip-{$log.ip|escape:'html':'UTF-8'}.html" target="_blank">{l s='View location' mod='ets_contactform7'}</a>
                                            {if !$log.black_list}
                                                <a class="btn btn-default addtoblacklist " data-ip="{$log.ip|escape:'html':'UTF-8'}" href="{$action|escape:'quotes'}&addtoblacklist={$log.ip|escape:'html':'UTF-8'}">{l s='Add to blacklist' mod='ets_contactform7'}</a>
                                            {else}
                                            <span title="{l s='IP added to blacklist' mod='ets_contactform7'}"><i class="icon icon-user-times"></i></span>
                                            {/if}
                                        </td>
                                    </tr>
                                {/foreach}
                        ...
                        <form action="{$action|escape:'quotes'}" enctype="multipart/form-data" method="POST">
                            ...
                            <button class="clear-log btn btn-default" type="submit" name="clearLogSubmit">{l s='Clear all view logs' mod='ets_contactform7'}</button>
                        </form>
                    {else}

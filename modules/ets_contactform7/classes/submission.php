...
class WPCF7_Submission {
	...
	private function submit() {
		...
		$this->meta = array(
			...
			'unit_tag' =>
				Tools::isSubmit('_etscf7_unit_tag') ? Tools::getValue('_etscf7_unit_tag') : '',
			'container_post_id' => Tools::isSubmit('_etscf7_container_post')
				? (int) Tools::getValue('_etscf7_container_post') : 0,
			'current_user_id' => (int)Context::getContext()->customer->id,
		);
		$contact_form = $this->contact_form;
        $listips = explode("\n",Configuration::get('ETS_CTF7_IP_BLACK_LIST'));
        $myip = Tools::getRemoteAddr();
        if($listips && in_array($myip,$listips))
        {
    ...
    private function validate() {
		...
		foreach ( $tags as $tag ) {
            ...
            if($type=='range')
                $type='number';
            $func= 'etscf7_'.$type.'_validation_filter';
			if(function_exists($func))
                $result = $func($result,$tag);
            else
                $result = etscf7_text_validation_filter($result,$tag);
		}

...
function etscf7_get_hangover( $name, $default = null ) {
	if ( ! etscf7_is_posted() ) {
		return $default;
	}
	...
	return Tools::isSubmit($name) ? ets_unslash(Tools::getValue($name) ) : $default;
}
...
function etscf7_quiz_validation_filter( $result, $tag ) {
	$name = $tag->name;

	$answer = Tools::isSubmit($name) ? etscf7_canonicalize( Tools::getValue($name) ) : '';
	$answer = ets_unslash( $answer );

	$answer_hash = ets_hash( $answer, 'etscf7_quiz' );

	$expected_hash = Tools::isSubmit('_etscf7_quiz_answer_' . $name)
		? (string) Tools::getValue('_etscf7_quiz_answer_' . $name)
		: '';
...
function etscf7_file_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
function etscf7_upload_tmp_dir() {
	return dirname(__FILE__).'/../views/img/etscf7_upload';
}
function etscf7_textarea_validation_filter( $result, $tag ) {
	//$type = $tag->type;
	$name = $tag->name;
...
function etscf7_text_validation_filter( $result, $tag ) {
	$name = $tag->name;

	$value = Tools::getValue($name) && !is_array(Tools::getValue($name))
		? trim( ets_unslash( strtr( (string) Tools::getValue($name), "\n", " " ) ) )
		: Tools::getValue($name);
...
function etscf7_checkbox_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
function etscf7_date_validation_filter( $result, $tag ) {
	$name = $tag->name;
	...
	if ( $tag->is_required() && '' == $value ) {
		$result->invalidate( $tag, etscf7_get_message( 'invalid_required' ) );
	} elseif ( '' != $value && ! etscf7_is_date( $value ) ) {
...
function etscf7_number_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
function etscf7_captcha_validation_filter( $result, $tag ) {
	//$type = $tag->type;
	$name = $tag->name;
	//$captchac = '_etscf7_captcha_challenge_' . $name;
...
function etscf7_select_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
function etscf7_acceptance_validation_filter( $result, $tag ) {
...
function etscf7_recaptcha_check_with_google( $spam ) {
	...
	$response_token = etscf7_recaptcha_response();
	$spam = ! $recaptcha->verify( $response_token );

	return $spam;
}
function etscf7_recaptcha_response() {

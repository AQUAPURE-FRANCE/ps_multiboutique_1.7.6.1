...
include_once(_PS_MODULE_DIR_.'nrtfootereditors/models/NrtFootEditorsHtml.php');

class Nrtfootereditors extends Module
{
	protected $config_form = false;
	private $pattern = '/^([A-Z_]*)[0-9]+/';
	private $spacer_size = '5';
	private $user_groups;
	const INSTALL_SQL_FILE = 'import_data_footer.sql';
	public function __construct()
	{
		$this->name = 'nrtfootereditors';
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('Footer Editor');
		$this->description = $this->l('Required by author: AxonVIP.');
		$this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
		$this->config_name = 'NRT_footer';
	...
	public function install()
	{
		...
		$success = parent::install() 
			...
			&& $this->registerHook('displayFooterCustom')
			&& $this->addTab();
		return $success;	
	}
	...
	{
		$str = file_get_contents($this->getLocalPath().'template_footer.csv');
	...
	public function addTab()
	{
        ...
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminNrtThemeFooterEditor";
        $tab->name = array();
        foreach (Language::getLanguages() as $lang) {
            $tab->name[$lang['id_lang']] = "Manage Footer Editor";
        }
    ...
    {
        $id_tab = Tab::getIdFromClassName('AdminNrtThemeFooterEditor');
		$parentTabID = Tab::getIdFromClassName('AdminNrtMenu');
        $tab = new Tab($id_tab);
        $tab->delete();
	...
	protected function createTables()
	{
		...
		$res = Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'nrtfooteditors_html` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'nrtfooteditors_htmlc` (
		...
		/* custom html lang */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'nrtfooteditors_htmlc_lang` (
	...
	protected function deleteTables()
	{
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'nrtfooteditors_html`, `'._DB_PREFIX_.'nrtfooteditors_htmlc`, `'._DB_PREFIX_.'nrtfooteditors_htmlc_lang`;
		');
	}
 ...
 public function installCustomHtml() {
  ...
  return true;
	...
	public function installTable() {
		...
		if (!file_exists(_PS_THEME_DIR_. 'modules/nrtfootereditors/'. self::INSTALL_SQL_FILE))
			return (false);
		else if (!$sql = file_get_contents(_PS_THEME_DIR_. 'modules/nrtfootereditors/'. self::INSTALL_SQL_FILE))
			return (false);
			if (file_exists(_PS_THEME_DIR_. 'link_default.php')){
				require_once(_PS_THEME_DIR_. 'link_default.php');
				if(isset($link_default)){
					$sql = str_replace($link_default, __PS_BASE_URI__, $sql);	
				}}
		$sql = str_replace('ps_', _DB_PREFIX_, $sql);
		$sql = preg_split("/;\s*[\r\n]+/", $sql);
		foreach ($sql as $query) {
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('addCustomHtml') || (Tools::isSubmit('id_html')  && !Tools::isSubmit('submitAddHtml') && NrtFootEditorsHtml::htmlExists((int)Tools::getValue('id_html'))))
		...
		elseif (Tools::isSubmit('exportConfiguration'))
		{
			...
			$file_name = 'export_footer_'.time().'.csv';
		...
		if (Tools::isSubmit('submitNrtfootereditorsModule'))
			$this->_postProcess();
...
$tab_='<ul class="nav nav-tabs" role="tablist">
  ...
  $demo_home1='<form class="import_demo" style="display:none;"  method="post" action="'.$this->context->link->getAdminLink('AdminModules', false).'&importConfigurationdemo1&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">	
		<button type="submit" class="btn btn-default btn-lg"><span class="icon icon-upload"></span> '.$this->l('Template Footer 1').'</button></form>';
  $demo_home2='<form class="import_demo" style="display:none;" method="post" action="'.$this->context->link->getAdminLink('AdminModules', false).'&importConfigurationdemo2&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">	
		<button type="submit" class="btn btn-default btn-lg"><span class="icon icon-upload"></span> '.$this->l('Template Footer 2').'</button></form>';
  $demo_home3='<form class="import_demo" style="display:none;" method="post" action="'.$this->context->link->getAdminLink('AdminModules', false).'&importConfigurationdemo3&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">	
		<button type="submit" class="btn btn-default btn-lg"><span class="icon icon-upload"></span> '.$this->l('Template Footer 3').'</button></form>';
  $demo_home4='<form class="import_demo" style="display:none;"  method="post" action="'.$this->context->link->getAdminLink('AdminModules', false).'&importConfigurationdemo4&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">	
		<button type="submit" class="btn btn-default btn-lg"><span class="icon icon-upload"></span> '.$this->l('Template Footer 4').'</button></form>';
  $import_data='<form class="import_demo" method="post" action="'.$this->context->link->getAdminLink('AdminModules', false).'&importDatabase&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">	
	...
	protected function renderForm()
	{
		...
		$helper->submit_action = 'submitNrtfootereditorsModule';
		...
		$helper->tpl_vars = array(
			...
			'fronteditor_link' => $this->context->link->getModuleLink('nrtfootereditors','Editor', array(
	...
	{
		Configuration::updateValue($this->config_name.'_content', urldecode(Tools::getValue('submenu-elements')));
		$this->generateCss();
	}
		...
		{	
			if(isset($element['row_s']))
			{
				if(isset($element['row_s']['bgw']) && $element['row_s']['bgw'])
					$css .= '#nrtfootereditors .content-'.$element['elementId'].'{
	...
	public function generateCss()
	{
		...
		if (Shop::getContext() == Shop::CONTEXT_GROUP)
			$my_file = $this->local_path.'css/footer_editors_g_'.(int)$this->context->shop->getContextShopGroupID().'.css';
		elseif (Shop::getContext() == Shop::CONTEXT_SHOP)
			$my_file = $this->local_path.'css/footer_editors_s_'.(int)$this->context->shop->getContextShopID().'.css';
	...
	{	
		...
		if (Tools::isSubmit('id_html') && NrtFootEditorsHtml::htmlExists((int)Tools::getValue('id_html')))
		{
			$tab = new NrtFootEditorsHtml((int)Tools::getValue('id_html'));
	...
	public function getAddHtmlFieldsValues()
	{
		...
		if (Tools::isSubmit('id_html') && NrtFootEditorsHtml::htmlExists((int)Tools::getValue('id_html')))
		{
			$html = new NrtFootEditorsHtml((int)Tools::getValue('id_html'));
			$fields['id_html'] = (int)Tools::getValue('id_html', $html->id);
			$fields['title'] = $html->title;
			
		}
		else
			$html = new NrtFootEditorsHtml();
	...
	public function renderHtmlContents(){
		...
		$tabs = NrtFootEditorsHtml::getHtmls();
	...
	private function _postValidationHtml()
	{
		...
		if (Tools::isSubmit('submitAddHtml'))
		{
			...
			{
				if (!Validate::isInt(Tools::getValue('id_html')) && !NrtFootEditorsHtml::htmlExists(Tools::getValue('id_html')))
		...
		elseif (Tools::isSubmit('delete_id_html') && (!Validate::isInt(Tools::getValue('delete_id_html')) || !NrtFootEditorsHtml::htmlExists((int)Tools::getValue('delete_id_html'))))
			$errors[] = $this->l('Invalid id_html');
	...
	private function _postProcessHtml()
	{
		...
		{
			$tab = new NrtFootEditorsHtml((int)Tools::getValue('delete_id_html'));
			$res = $tab->delete();
			if (!$res)
	...
	{
		if (Shop::getContext() == Shop::CONTEXT_GROUP){
			$this->context->controller->registerStylesheet('modules-nrtfootereditors', 'modules/'.$this->name.'/css/footer_editors_g_'.(int)$this->context->shop->getContextShopGroupID().'.css', ['media' => 'all', 'priority' => 1004]);
		}
		elseif (Shop::getContext() == Shop::CONTEXT_SHOP)
		{
			$this->context->controller->registerStylesheet('modules-nrtfootereditors', 'modules/'.$this->name.'/css/footer_editors_s_'.(int)$this->context->shop->getContextShopID().'.css', ['media' => 'all', 'priority' => 1004]);
		}
	}

	public function hookDisplayFooterCustom()
	{	
		$random = round(rand(1, max(3, 1)));
		
		if (!$this->isCached('nrtfootereditors.tpl', $this->getCacheId('nrtfootereditors|'.$random)))
		{
		...
		return $this->display(__FILE__, 'nrtfootereditors.tpl', $this->getCacheId('nrtfootereditors|'.$random));
	...
	{	
		...
		$custom_html = NrtFootEditorsHtml::getHtmls();
		return $custom_html;
	}
			...
			//set variouse links
				if(isset($node['contentType'])){
					...
					switch ($node['contentType']) {
						...
						{	
							$customhtml = new NrtFootEditorsHtml((int)$node['content']['ids'], $id_lang);

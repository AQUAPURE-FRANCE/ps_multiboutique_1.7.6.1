				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						...
						{if $smarty.foreach.categoryTreeBranch.first}  <ul class="nav nav-tabs">{/if} 
							<li class="nav-item"><a href="#content-tab-{$child.elementId}"  {if isset($child.tabtitle)}title="{$child.tabtitle}"{/if} data-toggle="tab">{if isset($child.tabtitle)}{$child.tabtitle}{else}{l s='Set tab title' mod='nrtfootereditors'}{/if}</a></li>

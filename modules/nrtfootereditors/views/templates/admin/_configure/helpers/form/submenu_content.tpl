	...
	<div data-element-type="1" data-depth="{$node.depth}" data-element-id="{$node.elementId}" class="row menu_row menu_row_element menu-element {if $node.depth==0} first_rows{/if} menu-element-id-{$node.elementId}">
		...
		<div data-element-type="4" data-depth="{$node.depth}" data-width-p="{$node.width_p}" data-width-t="{$node.width_t}" data-width-d="{$node.width_d}" data-contenttype="{$node.contentType}" data-element-id="{$node.elementId}" class="{if $node.width_p==13}phone-hidden{else}editors-col-xs-{$node.width_p}{/if} {if $node.width_t==13}tablet-hidden{else}editors-col-sm-{$node.width_t}{/if} {if $node.width_d==13}desktop-hidden{else}editors-col-md-{$node.width_d}{/if} menu_column menu_tabs menu-element menu-element-id-{$node.elementId}">
			...
			<div class="action-buttons-container content_click">
				...
				<button type="button" class="column-content-edit">{l s='Content' mod='nrtfootereditors'}</button>
			...
			<div class="dragger-handle btn btn-danger"><i class="icon-arrows "></i></a> {if $node.type==1}{l s='Row' mod='nrtfootereditors'}{elseif $node.type==2}{l s='Column' mod='nrtfootereditors'} {elseif $node.type==3}{l s='Tab' mod='nrtfootereditors'} {elseif $node.type==4}{l s='Tabs' mod='nrtfootereditors'}{/if}</div>
			...
			{if $node.type==4}
				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						{if $child.type==3}
							<li data-element-id="{$child.elementId}" data-element-type="3" class="nrtfootcontent-tab-li nrtfootcontent-tab-li-id-{$child.elementId}  {if $smarty.foreach.categoryTreeBranch.first} active{/if}"><a href="#nrtfootcontent-tab-id-{$child.elementId}"  data-toggle="tab">{foreach from=$languages item=language}
				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						{if $child.type==3}
							<div id="nrtfootcontent-tab-id-{$child.elementId}"  class="tab-pane  {if $smarty.foreach.categoryTreeBranch.first} active {/if}nrtfootcontent-element-id-{$child.elementId}">{include file="./submenu_content.tpl" node=$child frontEditor=$frontEditor}</div>
						{/if} 	
					{/foreach}
				{/if}
				</div> 

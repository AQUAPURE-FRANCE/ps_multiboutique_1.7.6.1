...
class NrtTestimonial extends ObjectModel{
	...
	public static $definition = array(
		'table' => 'nrttestimonials',
		'primary' => 'id_nrttestimonials',
		'multilang' => false,
		'multishop' => true,
		'fields' => array(
	...
	public function __construct($id = NULL, $id_lang = NULL, $id_shop = null){
		Shop::addTableAssociation('nrttestimonials', array('type' => 'shop'));
	...
	public function add($autodate = true, $null_values = false){
		...
		$res=true ;
		if ($this->position <= 0)
			$this->position = NrtTestimonial::getHigherPosition() + 1;
		$res = parent::add($autodate , $null_values);
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'nrttestimonials_shop`(`id_nrttestimonials`,`id_shop`)
	...
	public function delete(){
		...
		$res &= parent::delete();
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'nrttestimonials_shop`
			WHERE `id_shop` = '.(int)$this->id
		);
		if($res){
			if(file_exists(_NRT_TESTIMONIAL_MEDIA_DIR_.$this->media))
			@unlink(_NRT_TESTIMONIAL_MEDIA_DIR_.$this->media);
			return true;
		}
	}
	...
	public static function getAllTestimonials($p = 1, $n = false, $id = false, $excpt_id = false){
		...
		$sql= 'SELECT * FROM '._DB_PREFIX_.'nrttestimonials lt ';
		$sql .=' INNER JOIN '._DB_PREFIX_.'nrttestimonials_shop ls ON (lt.id_nrttestimonials = ls.id_nrttestimonials) ';
		$sql .=' WHERE lt.active = 1 AND ls.id_shop ='.$id_shop .( $id ? ' AND lt.id_nrttestimonials ='.(int)$id :'').
		( $excpt_id ? ' AND lt.id_nrttestimonials != '.(int)$excpt_id :'');
	...
	public function updatePosition($way, $position){
		if (!$res = Db::getInstance()->executeS('
			SELECT `id_nrttestimonials`, `position`
			FROM `'._DB_PREFIX_.'nrttestimonials`
			ORDER BY `position` ASC'
		))
		return false;
		foreach ($res as $testimonial)
		if ((int)$testimonial['id_nrttestimonials'] == (int)$this->id)
		...
		return (Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'nrttestimonials`
		...
		&& Db::getInstance()->execute('
			...
			WHERE `id_nrttestimonials` = '.(int)$moved_testimonial['id_nrttestimonials']));
	}
	/**
	...
	public static function cleanPositions()
	{
		...
		$return = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'nrttestimonials`
			SET `position` = '.(int)$i++.'
			WHERE `id_nrttestimonials` = '.(int)$value['id_nrttestimonials']);
		return $return;
	}
	...
	{
		$sql = 'SELECT MAX(`position`)FROM `'._DB_PREFIX_.'nrttestimonials`';

...
class NrttestimonialsViewsModuleFrontController extends ModuleFrontController {
	public $errors = array();
	public $success;
	public $identifier;
	public function __construct(){
		...
		$this->name ='nrttestimonials';
		$this->identifier = 'id_nrttestimonials';
		smartyRegisterFunction($this->context->smarty, 'function', 'testimonialpaginationlink', array('NrttestimonialsViewsModuleFrontController', 'getTestimonialPaginationLink'));
		}
	...
	public function listAllTestimoninals(){
		$this->addCSS(_MODULE_DIR_._NRT_TESTIMONIAL_FRONT_URL_.'css/style.css');
		...
		if ($id == 0){
			$alltestimoninals = NrtTestimonial::getAllTestimonials();
			$testimoninals = NrtTestimonial::getAllTestimonials($p, $n);
			...
			$testimoninals = NrtTestimonial::getAllTestimonials(1,false,$id,false);//view all and curent testimonial
			$other_testimoninals = NrtTestimonial::getAllTestimonials(1,false,false,$id); // view all other testimonial
			$page_other_testimonials = NrtTestimonial::getAllTestimonials($p,$n,false,$id); // item on page
	...
	public function formTestimoninals(){
		...
		$this->addJS(_MODULE_DIR_._NRT_TESTIMONIAL_FRONT_URL_.'js/validate_nrts.js');
		$tm_captcha = (int)$this->module->getParams()->get('captcha');
		$captcha_code = _MODULE_DIR_.'nrttestimonials/captcha.php';
		$loader_image = $video_vimeo = _MODULE_DIR_._NRT_TESTIMONIAL_FRONT_URL_.'img/loading.gif';
		$this->context->smarty->assign(array(
			'captcha' => $tm_captcha,
	...
	public function postValidation(){
		$this->validateRules('NrtTestimonial');
		if(Tools::isSubmit('submitNewTestimonial')){
			...
			$captcha = $_SESSION['nrttestimonials_captcha'];

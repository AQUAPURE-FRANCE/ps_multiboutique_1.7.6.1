<?php
include_once(_PS_MODULE_DIR_.'nrttestimonials/classes/NrtTestimonial.php');
include_once(_PS_MODULE_DIR_.'nrttestimonials/defined.php');
include_once(_PS_MODULE_DIR_.'nrttestimonials/nrttestimonials.php');
include_once(_PS_MODULE_DIR_.'nrttestimonials/classes/NrtFileUploader.php');
include_once(_PS_MODULE_DIR_.'nrttestimonials/libs/Params.php');
class AdminTestimonialController extends ModuleAdminController {
	public $bootstrap = true ;
	protected $position_identifier = 'id_nrttestimonials';
	public function __construct(){
		$this->table = 'nrttestimonials';
		...
		$this->fields_list = array(
			'id_nrttestimonials'=> array('title' => $this->l('ID'), 'width' => 20),
	...
	public function initPageHeaderToolbar()
	{
		...
		$this->page_header_toolbar_btn = array(
		    'options' => array(
				'href' => 'index.php?controller=AdminModules&token='.Tools::getAdminTokenLite('AdminModules') .'&configure=nrttestimonials&tab_module=others&module_name=nrttestimonials',
				'desc' => $this->l('Options'),
	...
	public function postProcess(){
		...
		if(isset ($_FILES['media']['name']) && ($_FILES['media']['name'])!= null ){
			$img_old = _PS_MODULE_DIR_ . 'nrttestimonials/img/' . $obj->media;
			if (file_exists($img_old))
			unlink($img_old);
			$upload = new NrtFileUploader($this->module, $_FILES['media']);
	...
	public function processForceDeleteImage()
	{
		...
		$update = new NrtTestimonial($obj->id);
		$update->media = null;
	...
	public function ajaxProcessUpdatePositions()
	{
		...
		$id_nrttestimonial = (int)(Tools::getValue('id'));
		$positions = Tools::getValue($this->table);
	foreach ($positions as $position => $value){
		$pos = explode('_', $value);
		if (isset($pos[2]) && (int)$pos[2] === $id_nrttestimonial){
			if ($nrttestimonial = new NrtTestimonial((int)$pos[2])){
				if (isset($position) && $nrttestimonial->updatePosition($way, $position))
					echo 'ok position '.(int)$position.' for carrier '.(int)$pos[1].'\r\n';
				else
					echo '{"hasError" : true, "errors" : "Can not update carrier '.(int)$id_nrttestimonial.' to position '.(int)$position.' "}';
			}else
					echo '{"hasError" : true, "errors" : "This carrier ('.(int)$id_nrttestimonial.') can t be loaded"}';
					break;
			}
		}
	}
	...
	else if (!Validate::isLoadedObject($object = new Nrttestimonial((int)Tools::getValue($this->identifier, Tools::getValue('id_nrttestimonials', 1)))))
	...
	public function renderForm($isMainTab = true) {
		...
		$obj = $this->loadObject(true);
		$this->context->controller->addJS(_MODULE_DIR_._NRT_TESTIMONIAL_ADMIN_URL_.'admin_testimonial.js');
		...
		if (in_array($obj->media_type,$img_types)){
			$media_desc = '<br/>
			<img id="image_desc" style="clear:both;border:1px solid black;width:100px;" alt="" src="'.__PS_BASE_URI__.'modules/nrttestimonials/img/'.$obj->media.'" />
			<br/>';
		} elseif (in_array($obj->media_type,$video_types)){
			$media_desc='<video width="320" height="240" controls>
			<source src="'.__PS_BASE_URI__.'modules/nrttestimonials/img/'.$obj->media.'" type="video/mp4" />
			</video>' ;
		}

...
{capture name=path}
    <span class="navigation-pipe">{$navigationPipe}</span>{l s='Testimonial' mod='nrttestimonials'}
{/capture}
<div class="content-testimonials block">
    <h4 class="title_block">{l s='Testimonial' mod='nrttestimonials'}</h4>
            <a class="button btn btn-default button-small" href="{$link->getModuleLink('nrttestimonials','views',['process' => 'form_submit'])}"><span>Submit Your Testimonials</span></a>
     {if $id != 0}
        <h3>{l s='Current testimonial' mod='nrttestimonials'}</h3>
    {/if}
            ...
            {if ($alltestimoninal.media_type)!=''}
                {if in_array($alltestimoninal.media_type,$image_type)}
                    <div class="img-content" >
                        <a class="alltestimoninal" href="{$modules_dir}nrttestimonials/img/{$alltestimoninal.media}">
                            <img src="{$modules_dir}nrttestimonials/img/{$alltestimoninal.media}" class="image-alltestimoninal" ></a>
                    </div>
                {/if}
                {if in_array($alltestimoninal.media_type,$video_type)}
                    <div class="img-content">
                        <video  controls class="image-alltestimoninal">
                            <source src="{$modules_dir}nrttestimonials/img/{$alltestimoninal.media}" type="video/mp4" />
                        </video>
                    </div>
                {/if}
        ...
        <div id="pagination" class="pagination">
                ...
                <ul class="pagination">
                    {if $page != 1}
                        ...
                        <li id="pagination_previous">
                            <a href="{testimonialpaginationlink p=$p_previous n=$nbpagination}" title="{l s='Previous' mod='nrttestimonials'}" rel="nofollow">&laquo;&nbsp;{l s='Previous' mod='nrttestimonials'}</a>
                        </li>
                    {else}
                        <li id="pagination_previous" class="disabled"><span>&laquo;&nbsp;{l s='Previous' mod='nrttestimonials'}</span></li>
                    {/if}
                    {if $page > 2}
                    ...
                    {if $alltestimoninals|@count > $page * $nbpagination}
                        ...
                        <li id="pagination_next"><a href="{testimonialpaginationlink p=$p_next n=$nbpagination}" title="Next" rel="nofollow">{l s='Next' mod='nrttestimonials'}&nbsp;&raquo;</a></li>
                    {else}
                        <li id="pagination_next" class="disabled"><span>{l s='Next' mod='nrttestimonials'}&nbsp;&raquo;</span></li>
                    {/if}
                </ul>
            {/if}
            {if $alltestimoninals|@count > 10}
                <form action="{$pagination_link}" method="get" class="pagination">
                    <p>
                        <input type="submit" class="button_mini" value="{l s='OK' mod='nrttestimonials'}" />
    ...
    {if $id !=0 && $other_testimoninals}
        <h3>{l s='Other testimonials' mod='nrttestimonials'}</h3>

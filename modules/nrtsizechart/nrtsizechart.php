...
include_once _PS_MODULE_DIR_ . 'nrtsizechart/models/SizeGuideNrt.php';

class NrtSizeChart extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->version = '1.0.0';
        $this->author = 'AxonVIP';
		$this->displayName = $this->l('Size Chart');
...
/* ------------------------------------------------------------- */
    ...
    private function _createTab()
    {
        ...
        $tab->name = array();
        foreach (Language::getLanguages() as $lang){
            $tab->name[$lang['id_lang']] = "Manage Sizechart";
        }
    ...
    protected function deleteTables()
    {
        ...
        foreach ($guides as $guide) {
            $to_del = new SizeGuideNrt($guide['id_guide']);
            $to_del->delete();
        }
    ...
    public function setDefaults()
    {
        foreach ($this->defaults as $default => $value) {
            if ($default == 'content') {
                ...
                $message_trads = array();
                $curren_path = $this->context->link->getMediaLink(_MODULE_DIR_ . $this->name . '/images/images.jpg');
                foreach (Language::getLanguages(false) as $lang) {
                    $message_trads[(int) $lang['id_lang']] = '<div class="row">
<div class="left-content-size col-md-6">
<div class="hover_banner_img"><a href="#"><img src="'.$curren_path.'" alt="" /></a></div>
</div>
<div class="right-content-size col-md-6">
<h2 class="title-measure">Measuring Tips to Assure The Best Fit</h2>
<ul class="text-measure">
<li><span>Measure your back size – wrap the measuring tape snugly around your ribcage, just under your bust line <em>1</em>, keeping it horizontal. This will give you your band size.</span></li>
...
<td>35/36</td>
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitGuide')) {
            ...
            if (Tools::getValue('id_guide')) {
                $guide = new SizeGuideNrt((int) Tools::getValue('id_guide'));
        ...
        elseif (Tools::isSubmit('delete_id_guide')) {
            $guide = new SizeGuideNrt((int) Tools::getValue('delete_id_guide'));
            $res = $guide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    protected function _postProcess2()
    {
        ...
        $this->_clearCache('nrtsizechart.tpl');
        $this->generateCss();
    }
    ...
    public function generateCss()
    {
        ...
        $css .= '
		#nrtsizechart{ width: ' . (int) Configuration::get($this->config_name . '_width') . 'px; height: ' . ((int) Configuration::get($this->config_name . '_height')) . 'px; }
		';
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        if (Validate::isLoadedObject($product = new Product((int)$params["id_product"]))) {
            ...
            $this->context->smarty->assign(array(
                'guides' => $this->getGuides(),
                'selectedGuide' => (int) SizeGuideNrt::getProductGuide((int)$params["id_product"]),
            ));
    ...
    public function hookActionProductSave($params)
    {
        ...
        if ($id_guide) {
            SizeGuideNrt::assignProduct($id_product, $id_guide);
        } else {
            SizeGuideNrt::unassignProduct($id_product);
        }
    ...
    public function _prepareHook($params)
    {
        ...
        $id_guide = SizeGuideNrt::getProductGuide((int) Tools::getValue('id_product'));
        $sh_global = Configuration::get($this->config_name . '_sh_global');

        if ($id_guide || $sh_global) {
            if ($id_guide) {
                $guide = new SizeGuideNrt((int) $id_guide, $this->context->language->id);
                $cache_id = 'nrtsizechart|' . (int) $id_guide;
            } else {
                $cache_id = 'nrtsizechart';
            }

            if (!$this->isCached('nrtsizechart.tpl', $this->getCacheId($cache_id))) {
                if ($id_guide) {
                    $this->smarty->assign(
                        array(
                            'guide' => $guide,
...
//                        'sh_measure' => $sh_measure,   
            ...
            return $this->display(__FILE__, 'nrtsizechart.tpl', $this->getCacheId($cache_id));
        }
    }
